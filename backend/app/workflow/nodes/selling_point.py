...
SYSTEM_PROMPT = (
    "You are a top-tier e-commerce marketing copywriter specializing in "
    "Xiaohongshu and Douyin social media content. Always respond with valid JSON only."
)


class SellingPointHandler(BaseNodeHandler):
    node_type = "selling_point"
    node_label = "Selling Point"
    ...
    async def execute(self, context: WorkflowContext) -> dict:
        ...
        variables = {
            ...
            "scenes_json": json.dumps(understanding.get("scenes", []), ensure_ascii=False),
            "selling_points_json": json.dumps(understanding.get("selling_points", []), ensure_ascii=False),
        }
        ...
        try:
            ...
            parsed = {"title": "", "marketing_copy": raw, "emotion_copy": ""}
